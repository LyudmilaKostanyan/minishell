...
int	redirection_out(t_vars *vars, t_cmds **cmds, int i)
{
	int	fd;
...
int	redirection_in(t_vars *vars, t_cmds **cmds, int i)
{
	...
	return (redirection_out(vars, cmds, i));
}
...
int	redirect_pipes(t_vars *vars, t_cmds **cmds, int count, int i)
{
	...
	{
		if (err_mes(dup2((*cmds)[i].pipe[1], 1) == -1, NULL, IO, vars))
			return (0);
		close((*cmds)[i].pipe[0]);
	}
	else if (i == count - 1 && !(*cmds)[i].red_in && count != 1)
	{
		if (err_mes(dup2((*cmds)[i - 1].pipe[0], 0) == -1, NULL, IO, vars))
			return (0);
